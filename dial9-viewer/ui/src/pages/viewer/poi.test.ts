...
describe("jump-marker wording", () => {
  ...
  it("names the worker and scopes the severity to the boxed span", () => {
    // Both halves matter: the box spans every lane (so nothing else says W0),
    // and its hard edges would otherwise read as a 17ms outage.
    expect(poiHighlightCaption(highlight)).toBe("W0 · 1.41ms off-CPU of 17.01ms");
  });
  ...
  it("drops the caption's severity claim to the share it really is", () => {
    ...
    expect(card.rows[1]!.value).toBe("17.01ms");

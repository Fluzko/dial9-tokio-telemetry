 ...
 *
 * Carries the two things the box itself cannot say: WHICH worker (it spans
 * every lane, so its shape attributes nothing) and how much of the boxed span
 * the severity actually accounts for. Without the "of 17.0ms" the hard box
 * edges read as the outage, overstating a 1.41ms stall by an order of
 * magnitude.
 */
export function poiHighlightCaption(h: PoiHighlight): string {
  const wall = formatHumanDuration(h.endNs - h.startNs);
  const severity = formatHumanDuration(h.severityNs);
  return `${workerLabel(h.worker)} · ${severity} ${severityNoun(h.kind)} of ${wall}`;
}

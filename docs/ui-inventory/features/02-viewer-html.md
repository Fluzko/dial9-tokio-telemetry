...
| G8b. Spawn-scope highlight | `[2026-09-04]` NEW: with the inspector Task tab scoped to "All from spawn" (`view.taskScope`), every poll of every task spawned at the selected task's location is tinted violet `#9575cd` - the timeline answer to "where else does this call site run?". Drawn BEFORE the waker highlight, so a task that is both still reads as the waker. Empty (no tint) for the single-task scope, no selection, or a task whose spawn location the trace never recorded. | Task tab scope switch. | `src/pages/viewer/task-flamegraph-model.ts` (`spawnScopeTaskIds`); `src/components/canvas/lanes/render.ts` (`drawSpawnScopeHighlight`) |

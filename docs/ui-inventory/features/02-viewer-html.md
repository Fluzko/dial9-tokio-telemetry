...
| N8b. Task scope + Flame | `[2026-09-04]` NEW: the inspector Task tab gains a `This task` / `All from spawn` switch and a `Flame` toggle. ONE scope drives both surfaces - the lanes tint the sibling tasks (G8b) and the flamegraph folds their samples - so the timeline and the profile always answer the same question. The single-task scope gathers samples off the task's own polls; the spawn-location scope reads `sample.spawnLoc` (stamped by `attachCpuSamples`), avoiding a poll flyweight per poll on a large trace. "All from spawn" is disabled for a task with no recorded spawn location. Both ride the URL (`task-flame`, `task-scope`). | Task tab controls. | `src/pages/viewer/task-flamegraph-model.ts`; `src/pages/viewer/inspector.ts` (`taskScopeControls`, `syncTaskFlamegraph`) |

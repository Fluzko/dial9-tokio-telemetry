...
| C2. POI filter dropdown | 5 options: Kernel Scheduling Delays, Long Polls (>1ms), Polls with CPU Samples, Wake->Poll Delays (>100us), Uninstrumented Polls. Change recomputes the POI list via `filterPointsOfInterest` and auto-jumps to the first match. `[2026-09-04]` The detectors RANK instead of thresholding: the fixed cutoffs are gone, the labels lose them ("Longest Polls", "Wake->Poll Delays"), and the rail lists the worst N (`poi.worstN`, one of 10/50/200 - a new `worst N` select beside the filter, URL param `issue-worst`) of the true match count it reports beside them. `spawn-delay`'s floor stays as the one explicit user filter, now defaulting to 0. | Toolbar `#poi-filter`. | `viewer.html:853-859`, `5025`, `2399-2411`; `trace_analysis.js:875-970`; `src/pages/viewer/poi.ts` (`POI_WORST_N_CHOICES`, `poisForFilter`) |
...
| H10. Selection overlay | `#selection-overlay` div spans full main-area height during shift/alt drag or keyboard selection; blue for shift, teal for alt (`setSelOverlayColor`); `pointer-events:none`; cleared on mouseup/Escape. Shift selections are owned by the sidebar and persist until it closes. `[2026-09-10]` (#876) VERTICAL EXTENT: the box spans the track stack MINUS the time ruler (`selectionSpan` over `.d9-tracks`), not `0..scrollHeight` - it no longer covers the ruler and hint chips above the tracks, nor trails past the last track into the empty column. Re-measured per render AND from a `ResizeObserver` on `.d9-tracks`, so it follows the lanes resize drag live; that drag withholds its height from the store until mouseup (so a shell re-render cannot fight it), leaving no store tick to ride. Landmarks come from `getBoundingClientRect`, never `offsetTop`, and the stack's bottom from the CONTAINER rather than its last child: #892 added a positioned overlay child to `.d9-tracks`, which made it the offset parent and silently slid the box back up over the ruler. The extent is mode-independent, so the Shift region and Alt zoom boxes get it too. `[2026-09-10]` (#876) ATTRIBUTION: the box spans every lane, so its shape says nothing about WHICH worker, and its hard edges would read as the outage when the severity is a fraction of the boxed wall time. So `selection.poiRange` carries worker + severity + kind alongside the range, and three surfaces word it (`poiHighlightCaption` / `poiHighlightTitle` / `poiHighlightSummary`): an amber caption pinned inside the box's leading edge ("W0 · 1.41ms off-CPU of 17.01ms", dropped below `CAPTION_MIN_WIDTH`), the inspector status line ("W0 descheduled period selected"), and a card above the inspector tabs (window / awake / off-CPU with its share). The card sits ABOVE the tabs because the period is neither a task nor a poll, so every tab is the wrong home; before it, this was the one POI kind whose click left the inspector reading "No selection". `[2026-09-09]` (#876) A third, lowest-precedence tier: `selection.poiRange`, an amber edge-tinted box marking the current issues-rail jump. Written only for a POI the lanes draw no bar for (`off-cpu-active` - a descheduled worker period is a property of a stretch of wall time, not of a poll or a park), so without it that jump moves the viewport and marks nothing. Rebuilt from the anchored POI on load, so a shared link lands boxed; cleared by Escape, by the inspector's Clear, and by any rail control that drops the current row. | Visible during selection. | `viewer.html:728-735`, `5133-5142`, `5183-5209`; `src/pages/viewer/selection-overlay.ts` (`activeSelectionRegion`), `src/pages/viewer/poi.ts` (`poiJump().highlight`) |

...
| G8b. Spawn-scope highlight | `[2026-09-04]` NEW: with the inspector Task tab scoped to "All from spawn" (`view.taskScope`), every poll of every OTHER task spawned at the selected task's location is tinted violet `#9575cd` - the timeline answer to "where else does this call site run?". The selected task is excluded so its own yellow (G6) survives. Drawn BEFORE the waker highlight, so a task that is both still reads as the waker. Empty (no tint) for the single-task scope, no selection, or a task whose spawn location the trace never recorded. | Task tab scope switch. | `src/pages/viewer/task-flamegraph-model.ts` (`spawnScopeTaskIds`); `src/components/canvas/lanes/render.ts` (`drawSpawnScopeHighlight`) |
...
| N8b. Task scope | `[2026-09-10]` the inspector Task tab carries a `This task` / `All from spawn` switch. ONE scope drives THREE surfaces - the stat rows re-read as family aggregates, the CPU flamegraph folds every sibling's samples, and the lanes tint those tasks (G8b) - so the numbers, the profile and the timeline always answer the same question. The single-task scope gathers samples off the task's own polls; the spawn-location scope reads `sample.spawnLoc` (stamped by `attachCpuSamples`), avoiding a poll flyweight per poll on a large trace. "All from spawn" is offered whenever the trace recorded a spawn location, which includes tasks spawned through raw `tokio::spawn` (tokio's task meta carries the location regardless of instrumentation). The profile is always rendered - it is the tab's only expandable surface, so a toggle bought a click and no choice. The scope rides the URL (`task-scope`). | Task tab controls. | `src/pages/viewer/task-flamegraph-model.ts`; `src/pages/viewer/task-scope-model.ts`; `src/pages/viewer/inspector.ts` (`taskScopeControls`, `familyStats`, `syncTaskFlamegraph`) |
